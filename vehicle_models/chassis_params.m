function chassis = chassis_params()
    ...
    chassis.m = 2.28;           % Vehicle mass, [kg]
    chassis.a = 0.138;          % Longitudinal dist from CoM to front axle, [m]
    chassis.b = 0.1;            % Longitudinal dist from CoM to rear axle, [m]
    chassis.h = 0;              % Vertical dist from CoM to axle plane, [m]
    chassis.xdot_o = 0;         % Initial longitudinal velocity, [m/s]

    %Lateral
    chassis.d = 0.027;          % Lateral dist from geometric centerline to CoM, [m]
    chassis.w = 0.1922.*[1 1];  % Track width, [m]
    ...
    %Yaw
    chassis.Izz = 2.5;          % Yaw polar inertia, [kg*m^2]
